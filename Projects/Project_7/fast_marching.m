...
% to change the color of the marker on a plot after done
...
%     I look for which of the NB has the smallest T
	ind_narrow_band=find(tipos==1); % es un indice lineal/it's a linear index
	if ~isempty(ind_narrow_band),
		[useless,ind_rel_min_T]=min(T(ind_narrow_band));
		ind_min_T=ind_narrow_band(ind_rel_min_T);
		% y lo paso a alive/and I pass it to alive
		tipos(ind_min_T)=0;
		%  fprintf('2');
		% paso a los vecinos a la NB/step to the neighbors to the NB
		[coord_x,coord_y]=ind2sub(siz,ind_min_T);
		punto=[coord_x,coord_y];
		for d=1:dim
			coord_vecinos(:,d)=punto(d)+vecinos(:,d);
			ind=find(coord_vecinos(:,d)>siz(d) | coord_vecinos(:,d)<=0 );
			%REVISAR: hay que arreglar el borde/CHECK: the edge has to be fixed
			coord_vecinos(ind,d)=punto(d)-(coord_vecinos(ind,d)-punto(d));
		end
		%fprintf('3');
		% busco los vecinos en far away y los paso a NB/
%         I look for the neighbors in far away and I pass them to NB
		lin_ind=sub2ind(siz,coord_vecinos(:,1),coord_vecinos(:,2));
...
%         I calculate the new T, only from the neighbors or from the whole 
% narrow band ??? !!! I think it's only in the neighbors first I calculate the m_i
